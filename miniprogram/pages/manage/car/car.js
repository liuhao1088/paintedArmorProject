...
Page({
  ...
  deleteAddr: function (e) {
    ...
    console.log(ind)
    wx.showModal({
      title: '删除 '+_this.data.list[ind].number_plate,
      content: '',
      confirmText: "删除",
      confirmColor: "red",
      success: function (res) {
        if (res.confirm) {
          ...
          wx.cloud.callFunction({
            ...
            wx.cloud.deleteFile({
              fileList: _this.data.list[ind].build_img,
              success: res => {
                // handle success
            ...
            wx.cloud.deleteFile({
              fileList: _this.data.list[ind].check_img,
              success: res => {
                // handle success
                console.log(res.fileList)
              },
  ...
  search(e) {
    ...
    if (e.detail.value == "") {
      ...
      db.collection("warranty").where(_.or([{
        mobile: {
  ...
  loadData:function(){
    ...
    db.collection('warranty').skip(skip).limit(20).orderBy("creation_date","desc").get().then(res => {
      let data = res.data;
      console.log(data)
